	...
	try:
		countab = pd.read_csv(countfp,
							 # names=['Sample','MEGID','Class','Mechanism','Group','Gene','Hits','GeneFrac','DUM','MY'], # add column names
							  names=['Sample','MEGID','Type','Class','Mechanism','Group','Hits','GeneFrac','DUM','MY'], 
		...
		countab_fin = countab.replace(countab.at[i, 'Sample'], samid) 
	except ValueError:
		pass	
...
def merge_normtab(mergeout=None, *args):
	try:
		...
		for file in args:
			...
			try:
				tabm = pd.merge(tab, tabm, how='outer', on = ['MEGID','Type','Class','Mechanism','Group']) 
...
if __name__ == "__main__":
	...
	make_RPKG_normtab(mcfp=args['mc'], 
					  ...
					  countfp=args['count'], 
					  outfp=args['out']) 
	merge_normtab(args['mergeout'], *args['mergein']) 
